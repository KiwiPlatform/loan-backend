...
public class Lead extends BaseEntity {
    ...
    @Column(name = "receptionist_name", nullable = true, length = 255)
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "medical_specialty_id", nullable = true)
    private MedicalSpecialty medicalSpecialty;

    @Column(name = "dni", nullable = false, length = 8)
    private String dni;

    @Column(name = "monthly_income", nullable = true, precision = 10, scale = 2)
    private BigDecimal monthlyIncome;

    @Column(name = "treatment_cost", nullable = true, precision = 10, scale = 2)
